...
def convert_to_npy(a_data_path, patch_size=(960, 960)):
    ...
    :param a_data_path: the image path
...
def to_2tuple(input):
    """
    Transform multiple size input into 2_tuple output
    """
...
def make_name(former_name, patch_size, patch_num):
    ...
    x = x - 1
    # every coordinate starts with 0
    x = former_x * ratio + x
...
def SVS_cut_to_patch(img, save_root, patch_size,
    ...
    try:
        ...
        # The resize process shall be taken when the resize_ratio value is not around 1
        if 1.1 > resize_ratio > 0.9:
            patch_size_num_0 = patch_size[0][0]
        else:
            patch_size_num_0 = int(patch_size[0][0] * resize_ratio)

        save_root_0 = os.path.join(os.path.join(save_root, str(patch_size[0][0])),
                                   class_name + '-' + str(patch_size[0][0]))
        ...
        for i in range(1, w // patch_size_num_0 - 1):
            for j in range(1, h // patch_size_num_0 - 1):
                ...
                patch = slide.read_region((i * patch_size_num_0, j * patch_size_num_0),
                                          0,
                ...
                img_color_check = patch.resize((1, 1), Image.ANTIALIAS)
                r, g, b = img_color_check.getpixel((0, 0))
                if r < 220 and g < 220 and b < 220 and r > 100 and b > 30 and r > g + 20:
                    ...
                    name_dir_3840[os.path.join(save_root_0,
                                               img_name + '-' + str(patch_size[0][0]) + '-' + str(i) + '-' + str(j)) + '-' + str(resize_ratio)] = img
                ...
                if S:
                    ...
                    for patch_2 in img_patches_2:
                        ...
                        if pick_patch(patch_2):
                            # due to the fact that there will be to many 96*96 under our patching standard
                            # the CPIA only sample 1/10 96*96 images into the final dataset
                            if k % 10 == 0:
...
def read_and_convert(data_root, save_root, suffix=None, patient_folder=False, L=True, M=True, S=False):
    ...
    for class_name in class_names:
        ...
        for img in svs_all_files:
            SVS_cut_to_patch(img, save_root, patch_size, class_name, name_dir_3840, name_dir_0, name_dir_1, name_dir_2,
                                patient_folder, L=L, M=M, S=S)


if __name__ == '__main__':
    read_and_convert(r'E:\Puzzle_Tuning_Datasets\Raw\WSI',
